...
for image_name in image_list:
    ...
    origin_image = resize(origin_image, (int(origin_image.shape[1] * 180 / origin_image.shape[0]), 180))

    # gray image
    gray_image = cvtColor(origin_image, COLOR_BGR2GRAY)
    gray_image = floyd_steinberg(gray_image)
    gray_image = resize(gray_image, (gray_image.shape[1] * 4, gray_image.shape[0] * 4))
    imwrite("./gray/" + image_name.split('.')[0] + ".png", gray_image)

    # cga image
    cga_image = atkinson(origin_image)
    cga_image = cga_convert(cga_image)
    cga_image = resize(cga_image, (cga_image.shape[1] * 4, cga_image.shape[0] * 4))
